...
Out of the 9 models, the best-performing models were optimized by using <code>Grid Search Cross-Validation</code>. Five different models were selected, and the performance of each model was visualized using a custom function `model_perf_visual()` that compares the actual and predicted values. The five models used were:
